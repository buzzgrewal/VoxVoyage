...
def main():
    print("\n=== Sonic StoryCraft CLI ===")
    voice_sample_path = input("Enter the path to your voice sample audio file: ").strip()
    if not os.path.exists(voice_sample_path):
        print("Error: The provided file path does not exist.")
        sys.exit(1)

    try:
        speaking_rate = float(input("Enter speaking rate (default 1.0, range 0.5 - 2.0): ") or "1.0")
        pitch = float(input("Enter pitch multiplier (default 1.0, range 0.5 - 2.0): ") or "1.0")
    except ValueError:
        print("Invalid input for parameters. Using default values (1.0).")
        speaking_rate, pitch = 1.0, 1.0

    story_text, current_node_id, options, audio_path = start_story(voice_sample_path, speaking_rate, pitch)
    print("\n--- Story Begins ---")
    ...
    play_now = input("Would you like to play the generated audio? (y/n): ").strip().lower()
    if play_now == "y":
        play_audio(audio_path)

    while options:
        try:
            choice = input("\nEnter the option number (or 'q' to quit): ").strip()
            if choice.lower() == "q":
                print("Exiting story. Goodbye!")
                break
        ...
        story_text, current_node_id, options, audio_path = advance_story(
            choice_index, current_node_id, voice_sample_path, speaking_rate, pitch
        )
        print(f"\nNarrative: {story_text}")
        if options:
            print("Available options:")
        ...
        play_again = input("Play the generated audio? (y/n): ").strip().lower()
        if play_again == "y":
            play_audio(audio_path)
